...
class TourNVController extends Controller
{
    public function getDanhSachTour(){
        $tour = tour::where('MaNV', Auth::id())->get();
    ...
    public function getthuchitour($MaCTTour){
        
        $thuchi= thuchi::where('MaCTTour',$MaCTTour)->get();
        
            return view('employee.qlTour.thuchitour',['thuchi'=>$thuchi],['MaCTTour'=>$MaCTTour] );

    }
    public function themthuchi(Request $request, $MaCTTour){
        
        $thuchi= new thuchi;
        $thuchi-> DichVu= $request -> DichVu;
        $thuchi-> MaNV= $request -> MaNV;
        $thuchi-> MaCTTour= $MaCTTour;
        $thuchi-> SoTien= $request -> SoTien;
        $thuchi-> NoiDung= $request -> NoiDung;
        $thuchi-> NgayTT= date(now());
        $thuchi-> LoaiThuChi= $request -> thuchi;
        $thuchi-> save();
    ...
    public function getDSKhachDi($MaCTTour){
        $hopdong = hopdong::where('MaCTTour',$MaCTTour)->where('TrangThai',1)->get();

        return view('employee.qlTour.DSKhachDi',['hopdong'=>$hopdong], ['MaCTTour'=>$MaCTTour]);
    }
    ...
    public function getThucTrangDV($MaDVTour){
        
      
        
        $dichvutour = dichvutour::where('MaDVTour',$MaDVTour)->first();
        
        return view('employee.qlTour.ThucTrangDV', ['dichvutour'=>$dichvutour] );
    ...
    public function postThemCTTour(Request $request, $MaTour ){
        ...
        if($request -> hasFile('LichTrinh'))
        {
            $file = $request->file('LichTrinh');

            $name = $file -> getClientOriginalName();

            $LT = str_random(4)."_".$name;
            while(file_exists('upload/lichtrinh'.$LT))
            {
                $LT = str_random(4)."_".$name;
            }
            $file->move("upload/lichtrinh", $LT);
            $chitiettour -> LichTrinh= $LT;
        }
        else
        {
    ...
    public function suaCTTour(Request $request, $MaCTTour ){
        ...
        $chitiettour ->  save();
         ...
         return redirect()->back() -> with('thongbao3', 'Update Thành Công');
     }
     else{
        $chitiettour -> NgayKhoiHanh= $request -> NgayKhoiHanh;
        $chitiettour -> NgayKetThuc= $request -> NgayKetThuc;
    ...
    public function postSuaCB(Request $request, $MaDVTour ){
        
        $dichvutour =  dichvutour::find($MaDVTour);
        $dichvutour -> SLNL= $request-> SLKCB;
        
        $dichvutour -> Gia= $request-> Gia;
        $dichvutour -> TongTien= $request-> TongTien;
     
        $dichvutour -> save();
        return redirect()->back() -> with('thongbao1', 'Update Thành Công');
    }
