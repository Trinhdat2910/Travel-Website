                  ...
                  <div style=" width: 100%;">
                      ...
                      <select class="form-control form-control-sm" id="tourmau" name="TourMau">
                     		@foreach(App\tour::all() as $t)
                     			<option value="{{$t -> MaTour}}">{{$t -> TenTour}} ({{$t -> MaTour}})</option>
                     		@endforeach
                    </select>
                    </div>
...
<script type="text/javascript">
	$(document).ready(function(){
        ...
        $.get('employee/ajax/TimTour/'+ Tour, function(data){
          $('#ngaykh').html(data);
		...
		$('.Timbtn').on('click',function(){
              ...
              if(macttour == ""){
              	alert('Bạn  Chưa  Chọn  Ngày Khởi Hành')
              		...
              		$.get('employee/ajax/TimTCTour/'+ macttour, function(data){
			          $('#dlThuChi').html(data);
    ...
    $('.Inbtn').on('click',function(){
              ...
              var macttour= $('#ngaykh').val();
                  $('#Inform').attr('action','employee/PDF/InTour/'+macttour);
                  $('#InModal').modal('show');
              }
