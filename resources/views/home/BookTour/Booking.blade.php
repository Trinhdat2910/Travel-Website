...
<form id="msform" style=" margin: 0 auto" action="home/BookTour/DatTour/{{ $chitiettour -> MaCTTour}}" method="POST" enctype="multipart/form-data">
  ...
  <fieldset>
    <div id="chooseDV">
        ...
        <div id="leftbooking">
            <div id="formSL" style="line-height: 10px;">
                ...
                <input type="number" name="SLNL" placeholder="Người Lớn" min="1" id="SLNL" required /> <span></span><br>
                <label>Trẻ Em</label><br>
                <input type="number" name="SLTE" placeholder="Từ 2 đến 12 tuổi" min="0" id="SLTE" value="0" /><span>Áp dùng từ 2 đến dưới 12 tuổi</span><br>
                <label>Trẻ Nhỏ</label><br>
                <input type="number" name="SLEB" placeholder="Dưới 2 tuổi"  min="0" id="SLEB"  value="0" /> <span> Áp dụng từ Dưới 2 tuổi</span>
                <input type="number" value="{{$chitiettour -> SL - $chitiettour -> SoChoDaDat}}" id="SoChoCon" style="display: none;" />
            </div>
            <div id="choosepay">
    ...
    <input type="button" name="next" class="next action-button" value="Tiếp Tục" id="nhapinfo" />
  </fieldset>
  <fieldset>
    ...
    <input type="button" name="previous" class="previous action-button" value="Trở Về" id="trove1" />

    <input type="button" name="next" class="action-button" value="Tiếp Tục" id="nhapkh" />
  </fieldset>
  ...
  <fieldset style="width: 600px;margin: 0 auto">
    ...
    <input type="submit" name="submit" class="action-button" value="Đặt Tour" data-id="{{ $chitiettour-> MaCTTour}} " />
  </fieldset>
</form>
    ...
    <script type="text/javascript">
      $(document).ready(function () {
        ...
        $('#nhapinfo').click(function(){
          var nl= parseInt($('#SLNL').val());
          var eb= parseInt($('#SLEB').val());
          var te= parseInt($('#SLTE').val());
          var tongsl= nl+ te + eb;
          var socho= $('#SoChoCon').val();
          ...
          else if(tongsl > socho)
            { alert("Tour Chỉ Còn "+socho+" chỗ"); }
        ...
        function addkh(){
          ...
          // var stt =document.getElementByClassByClassName("stt");
           ...
           '<table class ="tbhk">'+
            ...
            '<tbody>'+ 
            '<tr >'+
              '<td>Họ tên  <input type="text" class="hoten'+i+'" name="HoTen[]" required placeholder="Nhập Họ Tên"></td>'+
              '<td> Ngày Sinh  <input type="date" class="ngaysinh'+i+'" name="NgaySinh[]" required></td>'+
              '<td><p style="margin-bottom: 5px; line-height: 15px;">Giới tính </p> <select name="GioiTinh[]" class="gender">'+
                '<option value="Male"> Nam</option>'+
                '<option value="Female"> Nữ</option>'+
              '</select></td>'+
            ...
            '</tr>'+
            '<tr>'+
               '<td>Số Passport <input type="text" name="Passport[]" onKeyPress="return isNumberKey(event)" placeholder="Nhập số Passport" maxlength="9"></td>'+
                '<td>Số điện thoại  <input type="text" name="Tel[]" onKeyPress="return isNumberKey(event)" class="sdt'+i+'" placeholder="Nhập số điện thoại" maxlength="11"></td>'+
                 '<td>Địa Chỉ <input type="text" name="Address[]" class="diachi'+i+'" required placeholder="Nhập địa chỉ"></td>'+
            '</tr>'+
          '</tbody>'+
          '</table>';
            $('.dshk').append(table);
